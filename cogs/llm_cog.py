...
class LLMCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        ...
        # For remembering the code that was generated
    ...
    @commands.command(name="lastcode")
    @commands.dm_only()
    async def last_code(self, ctx: commands.Context):
        """Displays the last Python code block generated by the tool."""
        if not self.last_generated_code:
            embed = create_embed("No Code Found", "The code execution tool has not been used yet.", EmbedColors.INFO)
            await ctx.send(embed=embed)
            return

        # Format the code into a markdown block for Discord
        formatted_code = f"```python\n{self.last_generated_code}\n```"
        
        # Use the send_long_message helper in case the code is lengthy
        await send_long_message(ctx.channel, f"**Last Generated Code:**\n{formatted_code}")
